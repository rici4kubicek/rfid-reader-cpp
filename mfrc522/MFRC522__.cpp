 ...
 * Constructor.
 */
MFRC522::MFRC522(uint8_t slave, uint8_t aaa) { // SS is defined in pins_arduino.h, UINT8_MAX means there is no connection from Arduino to MFRC522's reset and power down input
} // End constructor
...
void MFRC522::PCD_ReadRegister(	PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
    ...
    //qDebug() << "Reading ")); 	Serial.print(count); qDebug() << " bytes from register."));
	...
	if (rxAlign) {		// Only update bit positions rxAlign..7 in values[0]
        ...
        buf3[0] = address;
        buf3[1] = 0xff;
        wiringPiSPIDataRW(0, buf3, 2);
        uint8_t value = buf3[1];
	...
	while (!uidComplete) {
		...
		while (!selectDone) {
			...
			if (currentLevelKnownBits >= 32) { // All UID bits in this Cascade Level are known. This is a SELECT.
                //qDebug() << "SELECT: currentLevelKnownBits=")); Serial.println(currentLevelKnownBits, DEC);
                ...
                //qDebug() << "ANTICOLLISION: currentLevelKnownBits=")); Serial.println(currentLevelKnownBits, DEC);
...
const __FlashStringHelper MFRC522::GetStatusCodeName(MFRC522::StatusCode code	///< One of the StatusCode enums.
    ...
    val = 0x1;
	switch (code) {
        case STATUS_OK:				return val;//F("Success.");
        case STATUS_ERROR:			return val;//F("Error in communication.");
        case STATUS_COLLISION:		return val;//F("Collission detected.");
        case STATUS_TIMEOUT:		return val;//F("Timeout in communication.");
        case STATUS_NO_ROOM:		return val;//F("A buffer is not big enough.");
        case STATUS_INTERNAL_ERROR:	return val;//F("Internal error in the code. Should not happen.");
        case STATUS_INVALID:		return val;//F("Invalid argument.");
        case STATUS_CRC_WRONG:		return val;//F("The CRC_A does not match.");
        case STATUS_MIFARE_NACK:	return val;//F("A MIFARE PICC responded with NAK.");
        default:					return val;//F("Unknown error");
	}
...
const __FlashStringHelper MFRC522::PICC_GetTypeName(PICC_Type piccType	///< One of the PICC_Type enums.
    ...
    val = 0x1;
	switch (piccType) {
        case PICC_TYPE_ISO_14443_4:		return val;//F("PICC compliant with ISO/IEC 14443-4");
        case PICC_TYPE_ISO_18092:		return val;//F("PICC compliant with ISO/IEC 18092 (NFC)");
        case PICC_TYPE_MIFARE_MINI:		return val;//F("MIFARE Mini, 320 bytes");
        case PICC_TYPE_MIFARE_1K:		return val;//F("MIFARE 1KB");
        case PICC_TYPE_MIFARE_4K:		return val;//F("MIFARE 4KB");
        case PICC_TYPE_MIFARE_UL:		return val;//F("MIFARE Ultralight or Ultralight C");
        case PICC_TYPE_MIFARE_PLUS:		return val;//F("MIFARE Plus");
        case PICC_TYPE_MIFARE_DESFIRE:	return val;//F("MIFARE DESFire");
        case PICC_TYPE_TNP3XXX:			return val;//F("MIFARE TNP3XXX");
        case PICC_TYPE_NOT_COMPLETE:	return val;//F("SAK indicates UID is not complete.");
		case PICC_TYPE_UNKNOWN:
        default:						return val;//F("Unknown type");
	}
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
		...
		if (firstInGroup) {
			// Print access bits
            qDebug() << " [ ";
            //Serial.print((g[group] >> 2) & 1, DEC); qDebug() << " ";
            //Serial.print((g[group] >> 1) & 1, DEC); qDebug() << " ";
		...
		if (group != 3 && (g[group] == 1 || g[group] == 6)) { // Not a sector trailer, a value block
            ...
            qDebug() << " Value=0x"; //Serial.print(value, HEX);
            qDebug() << " Adr=0x"; //Serial.print(buffer[12], HEX);
		}
        //Serial.println();
	}
